...
`index.html:516-603` is a modal titled **"Keyboard Shortcuts."** Its last section, "Buttons with
...
- It is **titled for a keyboard**, on an app whose primary device is an iPad with no keyboard.
  Six of its eight sections are irrelevant to a beginner, and `app.js:11171` concedes the modal
...
| Row | Says | Actually |
|---|---|---|
| `index.html:534` | Foul is a **"Black ✕"** | Renders navy — `rgba(0,50,120,0.88)`, `styles.css:965` |
| `index.html:567` | WP/PB — **"All runners advance 1"** | Opens a popup asking *who moved* unless exactly one runner is on (`applyRunnerEvent`, `app.js:5843`) |

The "Red ✕" for a strike is fair — `#8f0c14` (`styles.css:2275`) is a dark red. The foul row is
...
`showPositionPopup` (`app.js:7472`) was a 3×3 numeric grid, `1 P` … `9 RF`, laid out in
...
the single table `FIELD_MAP` (`app.js:7345`) and whose markup both mounts share through
`fieldMapMarkup` (`app.js:7455`).*

**B6 — Success is silent.**
After entry the cell shows `6-3` and nothing more. `showPlayToast` (`app.js:2519`) fires only for
refusals and caveats. A beginner has no way to tell whether the app understood them.

The renderer for this already exists: `describeCellForScreenReader` (`app.js:1344`) produces
*"Visiting, batting order 3, inning 5: 2B, 1 RBI, scored"* for the screen-reader live region, and
`updateCellAria` (`app.js:1462`) keeps it in sync from every paint path so it cannot drift. It is
...
`showRunnerPopup` (`app.js:4200`) is genuinely good design: it walks each runner by name, offers
...
*Fixed at step 0.3: the table `PLAY_TITLE` (`app.js:2233`) and its reader `playTitle`
(`app.js:2245`) name the play, and the popup titles itself "Single — Who Moved" in
`RUNNER_EVENT_TITLE`'s register (`app.js:5825`).*

**B9 — Two runner surfaces, two vocabularies.**
`showRunnerPopup` said `Hold / 2nd / 3rd / Home / Out 2nd`. The DP/FC/TP outcome popup, whose
chips are built by `ocChip` (`app.js:3541`), says `Hold 1st / Safe 2nd / Out at 2nd`. Same
...
- **H1 — The popups don't look like the app.** ~160 hardcoded hex values in `app.js` inline
  ...
  *Fixed at step 3: every one of those inline styles is gone, and a hex literal in generated
  popup markup now fails a test. The shell is `.jsp` (`styles.css:1186`) and the chips are
  `.jsp-chip` (`styles.css:1326`).*
- **H2 — The JS-built popup controls were never touch-audited.** The stylesheet carries a
  measured target table (`styles.css:1856`) covering the deck, lineup and linescore. It did
  ...
  button (≈19px), `padding:2px 7px` on "↻ Fix Stats" (`app.js:1101`, ≈18px). These are controls
...
- **H5 — No keyboard entry into the grid.** No `tabindex` on `.at-bat-cell`, and arrow
  navigation bails early without a prior tap (`app.js:10951`).
...
Replace the 3×3 keypad's *layout* (`app.js:7472`) with a positioned diamond-and-outfield
...
inline-SVG pattern (`diamondSVG` `app.js:487`; the scoreboard mini-diamond `index.html:157`).
...
Cheapest item here per unit of value, because `describeCellForScreenReader` (`app.js:1344`)
...
| | Fix | Why it leads |
|---|---|---|
| 0.1 | `index.html:567` — WP/PB row: "All runners advance 1" → "Pick who moved" | Actively misteaches a rule. Worst defect per character of fix. |
| 0.2 | `index.html:534` — foul "Black ✕" → navy | Wrong. |
