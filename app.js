...
/* Export PDF (Feature 15) — capture the two scorecards + game summary into a
   PDF that downloads automatically (no print dialog). Scorecards are landscape,
   the game summary is portrait; all three fit one US-Letter page each.
   Uses vendored html2canvas + jsPDF (see index.html / sw.js precache). */
function exportPDF() {
  if (typeof html2canvas === 'undefined' || !window.jspdf || !window.jspdf.jsPDF) {
    alert('PDF tools are still loading. If this is the first run while offline, connect to the internet once so they can cache, then try again.');
    return;
  }
  _exportPdfRun();
}

function _pdfNextFrame() {
  // Resolve after a paint, but fall back to a timer so a throttled rAF
  // (e.g. the tab is backgrounded mid-export) can't stall the whole export.
...
// Move live nodes into `container` (remembering where each came from), run fn,
// then restore them to their exact original positions. Moving the real nodes
// (vs. cloning) keeps every input value and event binding intact.
async function _pdfWithNodes(nodes, container, fn) {
  const homes = nodes.map(n => ({ node: n, parent: n.parentNode, next: n.nextSibling }));
  homes.forEach(h => container.appendChild(h.node));
  try {
    return await fn();
  } finally {
    homes.forEach(h => h.parent.insertBefore(h.node, h.next));
  }
}

// Inside html2canvas's throwaway clone: replace form controls with static text
// so filled-in values render exactly (html2canvas is weak on live <input> text),
// and let the scoring grid show its full width instead of an internal scrollbar.
function _pdfPrepClone(scope) {
  const view = scope.ownerDocument.defaultView;
  // Let the scoring grid show in full without an internal scrollbar. Only relax
  // overflow — do NOT touch its width, or the table's fixed columns collapse.
  scope.querySelectorAll('.grid-wrap').forEach(g => { g.style.overflow = 'visible'; });
  const COPY = ['fontFamily', 'fontSize', 'fontWeight', 'fontStyle', 'color', 'textAlign',
    'textTransform', 'letterSpacing', 'lineHeight', 'paddingTop', 'paddingRight',
    'paddingBottom', 'paddingLeft', 'borderBottomWidth', 'borderBottomStyle',
    'borderBottomColor', 'backgroundColor'];
  scope.querySelectorAll('input, select, textarea').forEach(el => {
    const t = (el.type || '').toLowerCase();
    if (t === 'checkbox' || t === 'radio') return;
    let text = '';
    if (el.tagName === 'SELECT') { const o = el.options[el.selectedIndex]; text = o ? o.textContent : ''; }
    else { text = el.value || ''; }
    const cs = view.getComputedStyle(el);
    const box = el.ownerDocument.createElement('div');
    box.textContent = text;
    COPY.forEach(p => { box.style[p] = cs[p]; });
    box.style.width = cs.width;
    box.style.height = cs.height;
    box.style.boxSizing = 'border-box';
    box.style.display = 'inline-flex';
    box.style.alignItems = 'center';
    box.style.justifyContent = cs.textAlign === 'center' ? 'center' : (cs.textAlign === 'right' ? 'flex-end' : 'flex-start');
...
// Replace inline <svg> with a rasterizable <img> of the same SVG (CSS variables
// resolved to concrete values). html2canvas mis-renders SVGs that use clip-paths
// or var() colors — e.g. the win-probability chart — but handles <img> fine.
function _pdfInlineSvgs(scope) {
  const doc = scope.ownerDocument;
  const rootStyle = doc.defaultView.getComputedStyle(doc.documentElement);
  const VARS = ['--gold', '--navy', '--accent', '--blue', '--heading', '--font',
    '--mono', '--text', '--text-light', '--card', '--border', '--border-light'];
  const vals = VARS.map(v => [v, rootStyle.getPropertyValue(v).trim()]).filter(p => p[1]);
  scope.querySelectorAll('svg').forEach(svg => {
    // Size from the viewBox (viewport-independent); fall back to a measured rect
    // or width/height attrs. Keeps the SVG's aspect ratio intact.
    const vb = (svg.getAttribute('viewBox') || '').split(/[\s,]+/).map(Number);
    const hasVB = vb.length === 4 && vb[2] > 0 && vb[3] > 0;
    const rect = svg.getBoundingClientRect();
    let w = hasVB ? vb[2] : 0, h = hasVB ? vb[3] : 0;
    if (rect.width > 1) { w = rect.width; h = hasVB ? rect.width * (vb[3] / vb[2]) : rect.height; }
    if (!w) { w = parseFloat(svg.getAttribute('width')) || 300; h = parseFloat(svg.getAttribute('height')) || 160; }
    // A data-URI SVG rasterizes at its own width/height attrs (default 300x150 if
    // absent), NOT the <img>'s CSS size. Set explicit pixel dims at 2x so the
    // whole chart renders crisply instead of collapsing.
    svg.setAttribute('width', Math.round(w * 2));
    svg.setAttribute('height', Math.round(h * 2));
    let s = new XMLSerializer().serializeToString(svg);
    vals.forEach(([k, v]) => { s = s.split('var(' + k + ')').join(v); });
...
function _pdfCaptureOpts(contentWidth, scopeId) {
  return {
    scale: 2,
    backgroundColor: '#ffffff',
    useCORS: true,
    windowWidth: Math.max(1440, Math.ceil(contentWidth) + 48),
    onclone: doc => { const s = doc.getElementById(scopeId); if (s) _pdfPrepClone(s); }
  };
}

// One scorecard page: shared game-info + linescore table, then that team's
// grid, pitchers, and spray chart. Excludes the pitch-entry toolbar and the
// live inning/count/bases panel (not meaningful on a finished scorecard).
async function _pdfCapturePanel(team, host) {
  const nodes = [
    document.querySelector('.game-info-card'),
    document.querySelector('.linescore-wrap > div'), // linescore table only
    document.getElementById('grid-' + team),
    document.querySelector('.' + team + '-bottom')
  ].filter(Boolean);
  return _pdfWithNodes(nodes, host, async () => {
    // Widen the host only if the grid (extra innings) needs more room, so the
    // full scorecard is captured without an internal scrollbar.
    const gw = host.querySelector('.grid-wrap');
    const need = gw ? gw.scrollWidth : 0;
    host.style.width = Math.max(1320, need + 44) + 'px';
    await _pdfNextFrame();
    const w = host.getBoundingClientRect().width;
    return html2canvas(host, _pdfCaptureOpts(w, host.id));
  });
}

// The game-summary card, exactly as the "Game Summary" modal shows it.
async function _pdfCaptureSummary() {
  showGameSummary(); // builds content + renders the win-prob chart synchronously
  const inner = document.getElementById('gs-inner');
  await _pdfNextFrame();
  await _pdfNextFrame();
  try {
    return await html2canvas(inner, {
      scale: 2, backgroundColor: '#ffffff', useCORS: true, windowWidth: 900,
      // Convert SVGs (the win-prob chart) inside the clone, where the viewport is
      // forced to 900px — html2canvas can't rasterize its clip-paths/var() colors.
      onclone: doc => { const s = doc.getElementById('gs-inner'); if (s) _pdfInlineSvgs(s); }
    });
  } finally {
...
// Encode a capture as JPEG and keep only its pixel dimensions, so the source
// canvas can be released before the next (memory-heavy) capture on mobile.
// JPEG (not PNG): the captures are large 2x rasters — PNG would balloon the file
// to tens of MB; on a white scorecard, high-quality JPEG stays crisp.
function _pdfToJpeg(canvas) {
  return { data: canvas.toDataURL('image/jpeg', 0.92), w: canvas.width, h: canvas.height };
}

function _pdfAddImagePage(pdf, img, orientation, isFirst) {
  const pageW = orientation === 'landscape' ? 792 : 612;
  const pageH = orientation === 'landscape' ? 612 : 792;
  if (!isFirst) pdf.addPage('letter', orientation);
  const M = 24;
...
function _pdfFilename() {
  const clean = s => (s || '').trim().replace(/[^\w-]+/g, '-').replace(/^-+|-+$/g, '');
  const v = clean(document.getElementById('info-visiting-team')?.value) || 'Visiting';
  const h = clean(document.getElementById('info-home-team')?.value) || 'Home';
  const d = clean(document.getElementById('info-date')?.value);
  let name = v + '-vs-' + h + (d ? '-' + d : '');
  return (name || 'scorecard') + '.pdf';
}

function _pdfShowOverlay() {
  let o = document.getElementById('pdf-overlay');
  if (!o) {
    o = document.createElement('div');
    o.id = 'pdf-overlay';
    o.style.cssText = 'position:fixed;inset:0;z-index:99999;background:rgba(15,23,42,0.88);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;';
    o.innerHTML = '<div style="width:44px;height:44px;border:4px solid rgba(255,255,255,0.25);border-top-color:#fff;border-radius:50%;animation:pdfspin 0.8s linear infinite"></div>'
      + '<div style="font-size:15px;letter-spacing:1.5px;text-transform:uppercase;font-weight:600">Generating PDF…</div>'
...
function _pdfHideOverlay() {
  const o = document.getElementById('pdf-overlay');
  if (o) o.style.display = 'none';
}

async function _exportPdfRun() {
  const overlay = _pdfShowOverlay();
  await _pdfNextFrame();
  const host = document.createElement('div');
  host.id = 'pdf-capture-host';
  host.style.cssText = 'position:fixed;left:-20000px;top:0;width:1320px;background:#fff;padding:16px 20px;z-index:-1;';
  document.body.appendChild(host);
  try {
    // Encode each capture to JPEG right away so its canvas can be freed before
    // the next capture — keeps peak memory low on iPad/iPhone.
    const imgV = _pdfToJpeg(await _pdfCapturePanel('visiting', host));
    const imgH = _pdfToJpeg(await _pdfCapturePanel('home', host));
    const imgS = _pdfToJpeg(await _pdfCaptureSummary());
    const { jsPDF } = window.jspdf;
    const pdf = new jsPDF({ orientation: 'landscape', unit: 'pt', format: 'letter' });
    _pdfAddImagePage(pdf, imgV, 'landscape', true);
    _pdfAddImagePage(pdf, imgH, 'landscape', false);
    _pdfAddImagePage(pdf, imgS, 'portrait', false);
